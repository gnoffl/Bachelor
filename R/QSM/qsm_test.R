...
neighborChordsObj <- function(model, ## Model to use for the evaluation
  ...
  for(j in 1:nrandom){
    ...
    ## Give resulting list
    if(is.null(result)){
      result <- list(manip_data = data[order(id, decreasing = FALSE)], ## Give manipulated dataset
      ...
      result <- list(manip_data = data[order(id, decreasing = FALSE)], ## Give LAST manipulated dataset
